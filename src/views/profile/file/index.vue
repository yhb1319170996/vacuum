  ...
  <div>
    <n-space>
      <div v-for="(stoveItem, i) in stoveConfigList" :key="i">
        ...
        <!-- 底部炉温配置 -->
        <n-card v-if="i <= 6" class="rounded-16px shadow-sm stove-panel">
          <n-space vertical>
            <!-- 温度对比 -->
            <n-grid x-gap="12" :cols="4">
              <n-gi>
    ...
    <div class="p-10px">
      ...
      <n-button type="primary" size="large" style="margin-left: 10px" @click="saveProfile">保存文件</n-button>
    </div>
...
import { savaAsFile, selectHeat } from '@/service/api/parm';
      ...
      const downSwitchObj = preHeatSwitchDownValue
        ...
        .concat(
          highHeatSwitchDownValue
            .slice(0, 2)
            .concat(vacuumHeatSwitchUpValue.slice(0, 1).concat(coldHeatSwitchDownValue.slice(0, 2)))
        );
...
async function saveProfile() {
  const param = {
    'Application.HMI_HomePage.h_rPreHeatZoneTempretureSV_UP': settingTemperatureArray.value.slice(0, 3),
    'Application.HMI_HomePage.h_rHighHeatZoneTempretureSV_UP': settingTemperatureArray.value.slice(3, 5),
    'Application.HMI_HomePage.h_rVacuumHeatZoneTempretureSV_UP': settingTemperatureArray.value.slice(5, 6),
    'Application.HMI_HomePage.h_rColdHeatZoneTempretureSV_UP': settingTemperatureArray.value.slice(6, 8),
    'Application.HMI_HomePage.h_iFanSpeedLevel_UP': upperFanArray.value
      .slice(0, 1)
      .concat(upperFanArray.value.slice(3, 4).concat(upperFanArray.value.slice(6, 7))),
    'Application.HMI_HomePage.h_iFanSpeedLevel_Down': lowerFanArray.value
      .slice(0, 1)
      .concat(lowerFanArray.value.slice(6, 7)),
    'Application.HMI_HomePage.h_rPreHeatZoneTempretureSV_DOWN': settingTemperatureArrayDown.value.slice(0, 3),
    'Application.HMI_HomePage.h_rHighHeatZoneTempretureSV_DOWN': settingTemperatureArrayDown.value.slice(3, 5),
    'Application.HMI_HomePage.h_rColdHeatZoneTempretureSV_DOWN': settingTemperatureArrayDown.value.slice(6, 8),
    'Application.HMI_HomePage.h_xPreHeatZoneTempretureOnOffSwitch_UP': topSwitch.value.slice(0, 3),
    'Application.HMI_HomePage.h_xHighHeatZoneTempretureOnOffSwitch_UP': topSwitch.value.slice(3, 5),
    'Application.HMI_HomePage.h_xVacuumHeatZoneTempretureOnOffSwitch_UP': topSwitch.value.slice(5, 6),
    'Application.HMI_HomePage.h_xColdHeatZoneTempretureOnOffSwitch_UP': topSwitch.value.slice(6, 8),
    'Application.HMI_HomePage.h_xPreHeatZoneTempretureOnOffSwitch_Down': bottomSwitch.value.slice(0, 3),
    'Application.HMI_HomePage.h_xHighHeatZoneTempretureOnOffSwitch_Down': bottomSwitch.value.slice(3, 5),
    'Application.HMI_HomePage.h_xVacuumHeatZoneTempretureOnOffSwitch_Down': bottomSwitch.value.slice(5, 6),
    'Application.HMI_HomePage.h_xColdHeatZoneTempretureOnOffSwitch_Down': bottomSwitch.value.slice(6, 8)
  };
  const success = await savaAsFile(param);
  console.log(param, 66);
}
function chickTrack() {
  if (showModalTrack.value === true) {
    showModalTrack.value = false;
  } else {
    showModalTrack.value = true;
  }
}
function chickOnN2() {
  if (showModalN2.value === true) {
...
function chickOnVacuum() {
  if (showModalVacuum.value === true) {
    showModalVacuum.value = false;
  } else {
    showModalVacuum.value = true;
  }
}
</script>
