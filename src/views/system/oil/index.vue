<template>
  <div>
    ...
    <n-modal v-model:show="coverShowTimeDown" class="rounded-10px" preset="card" title="请输入" style="width: 800px">
      ...
      <key-board-heat
        ...
        @onKeyPress="onKeyPressBurial"
      />
    </n-modal>
    <n-modal v-model:show="coverHeatWarning" class="rounded-10px" preset="card" title="请输入" style="width: 800px">
      ...
      <key-board-heat
        ...
        @onKeyPress="onKeyPressHeatWarning"
      />
    </n-modal>
    <n-modal v-model:show="coverBurialWarning" class="rounded-10px" preset="card" title="请输入" style="width: 800px">
      ...
      <key-board-heat
        ...
        @onKeyPress="onKeyPressBurialWarning"
      />
    </n-modal>
  </div>
</template>
...
function clickOn() {
  ...
  finalHeatDripOilTimeCountDown.value = '';
  minValue.value = 1;
  maxValue.value = 999;
}
function clickTimeCountDown() {
  ...
  finalBurialDripOilTimeCountDown.value = '';
  minValue.value = 1;
  maxValue.value = 999;
}
...
async function onKeyPress() {
  if (Number(finalHeatDripOilTimeCountDown.value) < Number(minValue.value)) {
    window.$notification?.error({
      title: '保存失败',
      content: `小于设定最小值!`,
      duration: 3000
    });
  } else if (Number(finalHeatDripOilTimeCountDown.value) > Number(maxValue.value)) {
    window.$notification?.error({
      title: '保存失败',
      content: `大于设定最大值!`,
      duration: 3000
...
async function onKeyPressBurial() {
  if (Number(finalBurialDripOilTimeCountDown.value) < Number(minValue.value)) {
    window.$notification?.error({
      title: '保存失败',
      content: `小于设定最小值!`,
      duration: 3000
    });
  } else if (Number(finalBurialDripOilTimeCountDown.value) > Number(maxValue.value)) {
    window.$notification?.error({
      title: '保存失败',
      content: `大于设定最大值!`,
      duration: 3000
...
async function onKeyPressHeatWarning() {
  if (Number(finalHeatWarning.value) < Number(minValue.value)) {
    window.$notification?.error({
      title: '保存失败',
      content: `小于设定最小值!`,
      duration: 3000
    });
  } else if (Number(finalHeatWarning.value) > Number(maxValue.value)) {
    window.$notification?.error({
      title: '保存失败',
      content: `大于设定最大值!`,
      duration: 3000
...
async function onKeyPressBurialWarning() {
  if (Number(finalHeatWarning.value) < Number(minValue.value)) {
    window.$notification?.error({
      title: '保存失败',
      content: `小于设定最小值!`,
